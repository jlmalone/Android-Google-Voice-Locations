...
public class LocationPhoneEnablePreference {
	...
	private final static Map<String,LatLng> locationMap = new HashMap<String,LatLng>();
	...
	public static void loadFromDB(Context context)
    {
		...
		try
        {
			...
			if(c.isFirst()){
				do
                {
					   ...
					   locationMap.put(c.getString(0),new LatLng(c.getInt(3),c.getInt(4)));
		...
		locationMap.put("Elsewhere", new LatLng(0,0));
	...
	public static void updatePreference(String location, String phone, int preference, int lat, int lon) throws Exception{
		...
		locationMap.put(location,new LatLng(lat,lon));
